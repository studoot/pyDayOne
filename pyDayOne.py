...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        try:
            saveFileDir = os.path.join(shell.SHGetFolderPath(
                0, shellcon.CSIDL_APPDATA, None, 0), 'pyDayOne')
        except ImportError:
        ...
        try:
            ...
            if platform.system() == 'Windows':
                ...
                if os.path.exists(directory):
                    ...
                    wx.MessageBox('Windows Dropbox folder not found. Please select the entries directory of Day One. It will be under Dropbox -> Apps -> Day One -> Journal.dayone -> entries',
                                  'Error', wx.OK | wx.ICON_EXCLAMATION)
                    dialog = wx.DirDialog(
                        None, "Choose the Day One entries directory:", style=wx.DD_DEFAULT_STYLE | wx.DD_NEW_DIR_BUTTON)
                ...
                if os.path.exists(directory):
                    ...
                    wx.MessageBox('Linux Dropbox folder not found. Please select the entries directory of Day One. It will be under Dropbox -> Apps -> Day One -> Journal.dayone -> entries',
                                  'Error', wx.OK | wx.ICON_EXCLAMATION)
                    dialog = wx.DirDialog(
                        None, "Choose the Day One entries directory:", style=wx.DD_DEFAULT_STYLE | wx.DD_NEW_DIR_BUTTON)
                    if dialog.ShowModal() == wx.ID_OK:
                ...
                wx.MessageBox('Dropbox folder not found. Please select the entries directory of Day One. It will be under Dropbox -> Apps -> Day One -> Journal.dayone -> entries',
                              'Error', wx.OK | wx.ICON_EXCLAMATION)
                dialog = wx.DirDialog(None, "Choose the Day One entries directory:",
        ...
        for root, dirs, files in os.walk(self.directory):
            ...
            for file in files:
                ...
                if (('Creation Date' in this_entry) and ('Entry Text' in this_entry) and
                    ...
                    self.entries[this_entry['Creation Date'].strftime(
                        datetime_format)] = this_entry
        ...
        self.entry_list = wx.ListBox(
            panel, 26, wx.DefaultPosition, (170, 630), self.timestamp_list, wx.LB_SINGLE)
        self.text = wx.TextCtrl(
            panel, -1, '', size=(500, 630), style=wx.TE_MULTILINE)
    ...
    def OnSave(self, event):
        ...
        saveToPath = os.path.join(
            self.directory, selected_entry['UUID']+'.doentry')
    ...
    def OnTimer(self, event):
        ...
        self.time.SetLabel(time.strftime("%H:%M:%S", print_time))
